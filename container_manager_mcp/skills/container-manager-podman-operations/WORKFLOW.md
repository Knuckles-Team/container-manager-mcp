...
2. **Create a pod and inspect it.**
   ```
   cm_podman action=podman_pod_create pod_name=web-pod image=nginx@sha256:<digest>
   cm_podman action=podman_pod_list
   cm_podman action=podman_pod_inspect pod_name=web-pod
   ```
3. **Verify health and logs.**
   ```
   cm_podman action=podman_health_check container_id=<id> config={"test":["CMD","curl","-f","[configured-endpoint]
...
cm_podman action=podman_play_kube_yaml yaml_path=[REDACTED_POSIX_LOCAL_PATH]
```
...
```
cm_podman action=podman_checkpoint container_id=<id> checkpoint_dir=[REDACTED_POSIX_LOCAL_PATH]
# ... host maintenance ...
cm_podman action=podman_restore container_id=<id> checkpoint_dir=[REDACTED_POSIX_LOCAL_PATH]
```
Create an isolated pod network with an explicit subnet:
```
cm_podman action=podman_network_create network_name=web-net driver=bridge subnet=[REDACTED_IPV4]/24
```
