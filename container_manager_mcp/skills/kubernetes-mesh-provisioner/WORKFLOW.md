...
On the manager node **R820 ([REDACTED_IPV4])**:
...
- Otherwise install the RKE2 agent, write `/etc/rancher/rke2/config.yaml` with
  `server: [configured-endpoint]`, the join token, and node labels; then
...
- For the ingress VIP, configure a `CiliumLoadBalancerIPPool` + `L2Announcement`
  (e.g. reuse `[REDACTED_IPV4]` so Technitium's `*.arpa → VIP` needs no change).

### Step 4: GPU Device Plugin
On GPU nodes **GR1080 ([REDACTED_IPV4])** and **GB10 ([REDACTED_IPV4], if joined)**:
...
- **CM-MCP itself needs three things to drive the cluster**: the `[mcp]`
  ...
  `[REDACTED_POSIX_LOCAL_PATH] and the relevant `K8S*TOOL=True` env vars set
