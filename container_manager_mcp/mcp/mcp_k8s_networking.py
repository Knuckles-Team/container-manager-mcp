...
def register_k8s_networking_tools(mcp: FastMCP):
    ...
    async def cm_k8s_networking(
        ...
        name: str | None = Field(default=None, description="Resource name (ingress class, network policy, service)"),
        spec: dict | None = Field(default=None, description="Specification for advanced networking operations"),
        rules: list | None = Field(default=None, description="NetworkPolicy rules for update_network_policy_rules"),
        ...
        service_name: str | None = Field(default=None, description="Service name for DNS endpoint lookup"),
        target: str | None = Field(default=None, description="Target for DNS connectivity test"),
        service_spec: dict | None = Field(default=None, description="Full spec dict for create_k8s_service"),
        service_ports: list | None = Field(default=None, description="Ports list for create_k8s_service"),
        service_selector: dict | None = Field(default=None, description="Selector for create_k8s_service"),
        service_type: str = Field(default="ClusterIP", description="Service type for create_k8s_service"),
        ...
        try:
            ...
            if action == "list_ingress":
                ...
                return await run_blocking(
                    manager.create_ingress, name=ingress_name, namespace=namespace, spec=ing_spec
                )
            ...
            # Network policies
                ...
                return await run_blocking(
                    manager.create_networkpolicy, name=netpol_name, namespace=namespace, spec=np_spec
                )
            ...
            # DNS debugging
                ...
                return await run_blocking(manager.check_dns_resolution, namespace, pod_name, hostname)
